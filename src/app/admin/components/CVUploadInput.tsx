...
interface CVUploadInputProps {
    source: string
    label: string
    customNameSource: string
    fileNameSource?: string
    required?: boolean
}

export const CVUploadInput = ({
    source,
    label,
    customNameSource,
    ...
    const { field: customNameField } = useInput({ source: customNameSource })
    const { field: fileNameField } = useInput({ source: fileNameSource })
    ...
    const uploadFile = async (file: File) => {
        ...
        try {
            const customName = customNameField.value || file.name.replace('.pdf', '')
            const formData = new FormData()
            formData.append('file', file)
            formData.append('customName', customName)

            // Upload vers /api/upload avec le nom personnalisé
            ...
            if (response.ok) {
                ...
                fileNameField.onChange(originalFileName)
                // Mettre à jour le customName si ce n'était pas déjà défini
                if (!customNameField.value) {
                    customNameField.onChange(customName)
                }
            } else {
            ...
            {!field.value ? (
                ...
                <div className="relative border border-gray-300 dark:border-gray-600 rounded-lg p-4 bg-white dark:bg-gray-800">
                    <div className="flex items-start gap-4">
                        <div className="flex-shrink-0 w-16 h-16 bg-gray-100 dark:bg-gray-700 rounded-md overflow-hidden flex items-center justify-center">
                            <FileText className="w-8 h-8 text-red-500" />
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">
                                {displayFileName || fileNameField.value || 'CV uploadé'}
                            </p>
